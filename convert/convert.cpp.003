...
double c2f( double c )
{
	...
	if ( c > -273.15 ) {	// only convert a value greater than aboslute-zero
		f = 32.0 + 9.0 / 5.0 * c;
	}
	return f;
}
...
double f2c( double f )
{
	...
	if ( f > -459.67 ) {	// only convert a value greater than aboslute-zero
		c = 5.0 / 9.0 * (f - 32.0);
	}
	return c;
}
...
int main()
{
	...
	//
	double c[6] = {-273.16, -273.15, 0.0, 37.0, 100.0, 200.0};	// 6 test cases for c to f conversion
	double f[6] = {-459.68, -459.67, 32.0, 98.6, 212.0, 392.0};	// 6 test cases for f to c conversion

	for ( int i = 0; i<=5; i++ ) {
		// run testing for c to f conversion
		...
		// run testing for f to c conversion
